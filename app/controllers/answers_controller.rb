...
class AnswersController < ApplicationController
  ...
  def edit
    ...
    question_answers = QuestionAnswer.where(question: @questionnaire.questions, user_id: params[:user_id])
  ...
  def form_set
    ...
    @questionnaire = Questionnaire.find(params[:questionnaire_id])
    @options = @questionnaire.questions.each_with_object({}) do |question, options|
      category = question.category
      if category == 'selectbox' || category == 'checkbox' || category == 'radio'
        options[question.id.to_s] = question.question_choices
      end
    end
  end
